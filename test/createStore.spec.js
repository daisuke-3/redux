...
describe("createStore", () => {
  ...
  it("only accepts plain object actions", () => {
    const store = createStore(reducers.todos);
    expect(() => store.dispatch(unknownAction())).toNotThrow();
  ...
  it("does not throw if action type is falsy", () => {
    const store = createStore(reducers.todos);
    expect(() => store.dispatch({ type: false })).toNotThrow();
    expect(() => store.dispatch({ type: 0 })).toNotThrow();
    expect(() => store.dispatch({ type: null })).toNotThrow();
    expect(() => store.dispatch({ type: "" })).toNotThrow();
  });
  ...
  it("throws if enhancer is neither undefined nor a function", () => {
    ...
    expect(() => createStore(reducers.todos, undefined, x => x)).toNotThrow();

    expect(() => createStore(reducers.todos, x => x)).toNotThrow();

    expect(() => createStore(reducers.todos, [])).toNotThrow();
  ...
  it("throws if nextReducer is not a function", () => {
    ...
    expect(() => store.replaceReducer(() => {})).toNotThrow();
  });
